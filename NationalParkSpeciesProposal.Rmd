...
```
``` {r}
number_nativness_df <- most_visited_nps_species_data_cleaned %>%
...
set.seed(221)
null_dist_nativeness <- number_nativness_df %>%
  filter(group != "middle") %>%
 specify(response = nativeness, explanatory = group,
 success = "Native") %>%
 hypothesize(null = "independence") %>%
 generate(100, type = "bootstrap") %>%
 calculate(stat = "diff in props",
 order = c("top5", "bottom5"))
